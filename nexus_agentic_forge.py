...
async def evaluer_et_formatter_juge(historique, texte_appelant_concatene, domaine_initial):
    ...
    try:
        # Utilisation du Structured Output (Pydantic) via le nouveau SDK
        ...
        print(f"⚠️ Erreur lors de l'évaluation du Juge : {e}")
        return None
...
async def worker_generation(id_worker, nb_conversations, resultats):
    for i in range(nb_conversations):
        ...
        try:
            ...
            print(f"❌ [Worker {id_worker}] Erreur critique lors de la simulation : {e}")


async def main():
    print("🚀 Démarrage de la Forge Agentique NEXUS (Mode Asynchrone Strict)...")

    # Paramètres de production optimisés pour éviter les limites de quota (Rate Limits)
    NB_WORKERS = 4
    CONVS_PER_WORKER = 3
    ...
    if resultats_valides:
        ...
        # Colonnes strictement alignées pour le modèle final
